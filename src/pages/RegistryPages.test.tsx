...
describe('Marketplace public states', () => {
  it('renders an empty state with a registration CTA', async () => {
    ...
    expect(await screen.findByRole('heading', { name: '아직 공개된 Agent가 없습니다.' })).toBeInTheDocument()
  ...
  it('renders an API error and retries the list query', async () => {
    listAgentsMock.mockRejectedValue(new Error('목록 API 오류'))
    renderWithQuery(<AgentsPage />)
...
describe('Agent detail actions', () => {
  it('publishes and disables versions while invalidating the relevant queries', async () => {
    ...
    await waitFor(() => expect(disableAgentVersionMock).toHaveBeenCalledWith('version-id', expect.any(Object)))
...
describe('Agent registration flow', () => {
  it('invalidates the Marketplace before navigating after registration', async () => {
    ...
    fireEvent.change(screen.getByLabelText(/Developer ID/), { target: { value: '123e4567-e89b-12d3-a456-426614174000' } })
    fireEvent.change(screen.getByLabelText(/^Slug/), { target: { value: 'demo-agent' } })
