...
export function LandingPage() {
  ...
  async function startDemo() {
    ...
    const destination = returnTo ?? (developerAccess ? '/developer/revenue' : '/marketplace')
    const existing = currentDemoAccess()
    if (existing) {
      if (developerAccess) setDisplayMode('developer')
    ...
    try {
      ...
      setDisplayMode(developerAccess ? 'developer' : 'easy')
      navigate(destination, { replace: true })
    } catch (error) {
      if (!mountedRef.current || controller.signal.aborted) return
