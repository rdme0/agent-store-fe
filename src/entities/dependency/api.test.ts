...
describe('Dependency API boundary', () => {
  ...
  it('maps create, update, delete, and quote operations to contract paths', async () => {
    fetchMock
      .mockResolvedValueOnce(new Response(JSON.stringify({ isSuccess: true, message: '요청이 성공했습니다.', errorCode: null, result: { id: 'd', sourceVersionId: 'v', targetAgentId: 'a', targetAgentCode: 'risk', versionConstraint: '>=1.0.0,<2.0.0', required: true, maxPriceAtomic: '1', maxCalls: 1, createdAt: '', updatedAt: '' } }), { headers: { 'Content-Type': 'application/json' }, status: 201 }))
      .mockResolvedValueOnce(new Response(JSON.stringify({ isSuccess: true, message: '요청이 성공했습니다.', errorCode: null, result: { id: 'd', sourceVersionId: 'v', targetAgentId: 'a', targetAgentCode: 'risk', versionConstraint: '==2.0.0', required: false, maxPriceAtomic: '2', maxCalls: 2, createdAt: '', updatedAt: '' } }), { headers: { 'Content-Type': 'application/json' }, status: 200 }))
      .mockResolvedValueOnce(new Response(null, { status: 204 }))
      .mockResolvedValueOnce(new Response(JSON.stringify({ isSuccess: true, message: '요청이 성공했습니다.', errorCode: null, result: { id: 'q', rootVersionId: 'v', expiresAt: '2026-01-01T00:05:00.000Z', maxCostAtomic: '1000000', snapshot: { version: { id: 'v', agentId: 'a', agentCode: 'investment', semver: '1.0.0', endpoint: 'http://localhost:8090', priceAtomic: '1000000', network: 'eip155:84532', asset: 'USDC', payTo: '0x1' }, dependencies: [] }, warnings: [] } }), { headers: { 'Content-Type': 'application/json' }, status: 201 }))

    await createDependency('v', { targetAgentId: 'a', versionConstraint: '>=1.0.0,<2.0.0', maxPriceAtomic: '1' })
    await updateDependency('v', 'd', { versionConstraint: '==2.0.0', maxPriceAtomic: '2', maxCalls: 2 })
    await removeDependency('v', 'd')
    const quote = await createAgentQuote('investment', { versionConstraint: '>=1.0.0,<2.0.0' })
