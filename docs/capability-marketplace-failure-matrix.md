...
| DA-FE-02 | Expiry and 401 clear stored access and return the user to the landing CTA; a stale completion cannot restore a cleared token. The shell exposes no lifetime or exit controls. | `src/shared/auth/demoAccess.test.tsx` covers expiry cleanup and the access-ended event; generated-client integration covers 401 cleanup; Playwright asserts the shell has no lifetime or exit controls. |
